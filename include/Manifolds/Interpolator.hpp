...
class MInterpolator {
private:
  MInterpolator(const MInterpolator &that);
  MInterpolator &operator=(const MInterpolator &);
...
public:
  MInterpolator(std::size_t _codom_dim, std::size_t _num_intervals,
                const gsplines::basis::Basis &_basis);
  virtual ~MInterpolator();
...
gsplines::GSpline pw_polynomial_interpolation(
    const Eigen::Ref<const Eigen::VectorXd> _interval_lengths,
