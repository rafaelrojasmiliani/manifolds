...
///     Implementes Algorithm 24 from Kopriva . This is used to compute the
///     gauss lobatto points.
///
///     David A. Kopriva Implementing Spectral Methods for
///     Partial Differential Equations Algorithm 24: qAndLEvaluation
...
constexpr std::tuple<double, double, double> q_and_evaluation(double _t,
  ...
  double l_n = 0.0;
  double lprime_n = 0.0;
  double lprime_n_plus_1 = 0.0;
  double l_n_plus_1 = 0.0;
  double q = 0.0;
  double qprime = 0.0;
...
///  Implements Algorithm 25 fro Kopriva. Compute the gauss lobatoo points and
///  its weights
///
///    David A. Kopriva
...
std::pair<Eigen::Matrix<double, _n, 1>, Eigen::Matrix<double, _n, 1>>
legendre_gauss_lobatto_points_and_weights() {
  ...
  return std::pair<Eigen::Matrix<double, _n, 1>, Eigen::Matrix<double, _n, 1>>(
      std::move(points), std::move(weights));
}

template <long long _n>
Eigen::Matrix<double, _n, 1> legendre_gauss_lobatto_points() {
  Eigen::VectorXd result;
  std::tie(result, std::ignore) =
      legendre_gauss_lobatto_points_and_weights<_n>();
  return result;
}

template <long long _n>
Eigen::Matrix<double, _n, 1> legendre_gauss_lobatto_weights() {
...
/* template <long long _nglp, long long _n_intervals> */
/* Eigen::Matrix<double, _nglp * _n_intervals, 1> */
...
/*     result.segment(interval * _nglp, _nglp) = glw * local_interval_length
 * / 2.0; */
/*   } */
/*   return result; */
/* } */
...
/*     result.segment(interval * _nglp, _nglp) = glw * _intervals[interval]
 * / 2.0; */
/*   } */
/*   return result; */
/* } */
