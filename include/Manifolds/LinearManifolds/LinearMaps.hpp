...
public:
  ...
  virtual bool
  value_on_repr(const typename base_t::domain_t::facade_t &_in,
                typename base_t::codomain_t::facade_t &_out) const override {
    ...
    if constexpr (base_t::codomain_t::is_faithful)
      ...
      output_ptr = &(_out.repr());
    *output_ptr = this->crepr() * (*input_ptr);
    return true;
  }

  virtual bool diff_from_repr(const typename base_t::domain_t::facade_t &,
                              detail::dense_matrix_ref_t _mat) const {
    _mat = this->crepr();
    return true;
  }
};
...
public:
  ...
  virtual bool diff_from_repr(const typename base_t::domain_t::facade_t &,
                              detail::sparse_matrix_ref_t _mat) const {
    _mat.get() = this->crepr();
    return true;
  }
};
