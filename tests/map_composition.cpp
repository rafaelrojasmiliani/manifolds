...
TEST(MapComposition, DenseLinearMaps) {
  ...
  for (int _ = 0; _ < 10; _++) {
    ...
    auto v2_test_2 = T4::random();
    c4.diff(v1, v2_test_2, mat);

    EXPECT_TRUE(v2_ground_truth.isApprox(v2_test_2.crepr(), 1.0e-5))
        << "Error on derivative \n"
        << v2_ground_truth.transpose() << "\n"
        << v2_test.crepr().transpose() << "\n";
    ...
    EXPECT_TRUE(mat.isApprox(m4.crepr() * m4.crepr() * m4.crepr() * m3.crepr() *
        ...
        << "Error on derivative \n ++++++++++++++++++++++++++++++\n"
        << mat << "\n-----------------------------------------\n"
        << m4.crepr() * m4.crepr() * m4.crepr() * m3.crepr() * m2.crepr() *
               m1.crepr()
        << "\n ++++++++++++++++++++++++++++++++++ \n";
  }
}

TEST(MapComposition, SparseLinearMaps) {

  using T1 = DenseLinearManifold<100>;
  using T2 = DenseLinearManifold<160>;
  using T3 = DenseLinearManifold<200>;
  using T4 = DenseLinearManifold<100>;

  using M1 = SparseLinearMap<T1, T2>;
  using M2 = SparseLinearMap<T2, T3>;
  using M3 = SparseLinearMap<T3, T4>;
  using M4 = SparseLinearMap<T4, T4>;
...
TEST(MapComposition, MixedLinearMaps) {

  using T1 = DenseLinearManifold<2>;
  using T2 = DenseLinearManifold<560>;
  using T3 = DenseLinearManifold<100>;
  using T4 = DenseLinearManifold<3>;
