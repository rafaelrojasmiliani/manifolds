...
TEST(Robotics, DirectKinematics) {
  ...
  auto fk = ForwardKinematics<7>::from_urdf("./urdf/panda_arm.urdf", "panda_link0");
...
TEST(Robotics, Motion) {
  ...
  auto fk = ForwardKinematics<dim>::from_urdf("./urdf/panda_arm.urdf", "panda_link0");

  IntervalPartition<intervals> interval(0.0, 2.0);
  using Pol = PWGLVPolynomial<nglp, intervals, dim>;

  auto fk2 = Pol::Composition<DenseLinearManifold<7>>(fk);
