...
template <typename M>
void register_map_once(py::module &m, const std::string &name) {
  static bool firstCall = true;
  if (firstCall) {
    py::class_<M, MapTrampoline<M>, MapBase>(m, name.c_str()).def(py::init<>());
  }
  firstCall = false;
}
// this avoid to register several times the map from reals to a vector space
// when registering several partitions and gauss-lobatto points
template <typename M>
void register_manifold_once(py::module &m, const std::string &name) {
  static bool firstCall = true;
  if (firstCall) {
    py::class_<M, ManifoldBase>(m, name.c_str()).def(py::init<>());
  }
  firstCall = false;
}

template <typename M>
void register_user_map(py::module &m, const std::string &name) {
  ...
  py::class_<M, typename M::base_t>(m, name)
      .def(py::init<>())
      .def("clone", &M::base_t::clone)
      .def("move_clone", &M::base_t::move_clone);
}
...
template <typename M>
auto register_user_linear_map(py::module &m, const std::string &name) {
...
template <typename M>
void register_polynomial(py::module &m, const std::string &name) {
  ...
  // Add Polynomial
  py::class_<M, typename M::base_t>(m, name.c_str())
  ...
  register_user_linear_map<typename M::template Derivative<1>>(m, "Derivative")
  ...
  register_manifold_once<typename C::manifold_t>(m, "C" + name + "Manifold");
  register_map_once<typename C::map_t>(m, ("C" + name + "Map").c_str());
  py::class_<typename C::base_t, MapTrampoline<typename C::base_t>,
             typename C::map_t, typename C::manifold_t>(
      m, ("C" + name + "Clonable").c_str())
      .def(py::init<>())
      .def("clone", &C::base_t::clone)
      .def("move_clone", &C::base_t::move_clone);

  py::class_<C, typename C::base_t>(m, ("C" + name).c_str())
      .def(py::init<double, double>())
      .def_static("random", &C::random);

  // Define the inclusion of continuous polynomials

  register_user_linear_map<typename C::Inclusion>(m, "Inclusion")
      .def(py::init<const typename M::interval_partition_t &>());
}
...
PYBIND11_MODULE(pymanifolds, manifolds_module) {
  ...
  register_polynomial<PWGLVPolynomial<10, 10, 3>>(linear_manifolds,
                                                  "GLPolynomial");
  register_polynomial<PWGLVPolynomial<10, 10, 1>>(linear_manifolds,
                                                  "GLPolynomial");
}
